    ...
    public fun validate(pattern: String): PatternCheck {
        ...
        for (input in EVIL_INPUTS) {
            ...
            try {
                ...
                // Реализация регулярных выражений в JDK рекурсивна, и на выражениях вида
                // `(a|aa)+b` она не превышает бюджет, а падает по стеку. Сторож времени такое
                // не ловит по определению. Без этой ветки отказ уходил наружу и ронял **весь**
                // импорт правил, тогда как обещано отклонение поштучно.
                return PatternCheck.TooExpensive(
                    "выражение слишком глубоко вложено: проверка не укладывается в стек"
                )
            }
        }
