...
/**
 * Явный API, как в `:engine`.
 *
 * Весь `:core` и так написан с `public`/`internal` на каждом объявлении — но это было
 * соглашение, а соглашение держится на внимании. Здесь оно превращается в проверку сборкой:
 * граница между «частью горячего пути» и «деталью реализации» слишком важна, чтобы
 * поддерживаться глазами.
 */
kotlin {
    explicitApi()
}
