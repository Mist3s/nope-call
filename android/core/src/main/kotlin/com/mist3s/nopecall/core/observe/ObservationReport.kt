...
public data class ObservationReport(
    ...
    /**
     * Проверок на сети без VoLTE.
     *
     * Показывается рядом с числом подписей, потому что без этого «подписей 0» читается как
     * «оператор не передаёт», хотя звонок мог идти через 3G. Утверждать механизм («на 3G
     * подписи не бывает») по десятку звонков нельзя: подпись существует и в сетях с коммутацией
     * каналов. Наблюдение показывает совпадение, а вывод делает тот, кто смотрит на числа.
     */
    ...
    public fun toText(): String = buildString {
        ...
        if (withSignature == 0 && checksWithoutVolte > 0) {
            appendLine()
            appendLine("  Подписи в момент проверки не было ни разу, и $checksWithoutVolte")
            appendLine("  ${if (checksWithoutVolte == 1) "проверка пришлась" else "проверок пришлось"} на сеть без VoLTE (всего проверок $checks).")
            appendLine("  Связь вероятна — подпись передаётся в сигнализации, — но по такому")
            appendLine("  числу звонков она не доказана. Что известно точно: на этих звонках")
            appendLine("  правила по названию сработать не могли, нужны правила по номеру.")
        }
        appendLine()
