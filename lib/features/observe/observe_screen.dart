...
/// приходит ли операторская подпись к моменту проверки, в каком формате, куда вендор кладёт
/// данные, какова реальная задержка. Половина ответов видна уже здесь.
///
/// Чего экран **не** отвечает: досылается ли подпись после решения. Средство проверки звонков
/// вызывается один раз, обновлений после ответа не получает, а по системному журналу источник
/// названия не устанавливается — поэтому показатель выводится словами «не измеряется».
///
...
class _SignatureCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final lateShare = checks == 0 ? 0.0 : report.lateSignatures / checks;
    // Поздние названия неустановленного происхождения: название пришло из системного журнала,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _Line(
              'Подпись была в момент проверки',
              '${report.withSignature} (${(share * 100).round()} %)',
            ),
            // Ноль тут неотличим от «не измеряли», а прочитан будет как «оператор не
            // досылает». Канала наблюдения у средства проверки звонков нет: после ответа
            // система обновлений не присылает, а системный журнал источник не устанавливает.
            _Line(
              'Подпись пришла после решения',
              report.lateSignatures > 0
                  ? '${report.lateSignatures} (${(lateShare * 100).round()} %)'
                  : 'не измеряется',
            ),
            const SizedBox(height: 8),
            _Line(
              'Название было в момент проверки',
              '${report.namesAtDecision}',
            ),
            _Line('Название дописано позже', '${report.lateNames}'),
            _Line('Названия не было', '${report.withoutName}'),
            _Line('Скрытый номер', '${report.hiddenNumbers}'),
            if (report.lateSignatures > 0) ...[
              const SizedBox(height: 12),
              Text(
            ...
            // Причина отсутствия подписи известна точно и не зависит от сети, оператора
            // и версии Android: система не передаёт название приложениям проверки звонков.
            // Прежние формулировки («на 3G подписи не бывает», «мы спрашиваем раньше») были
            // выводами по косвенным признакам и оказались неверными.
            if (checks > 0) ...[
              const SizedBox(height: 12),
              Text(
                'Названия в момент проверки не бывает, и это не отказ оператора: система '
                'не передаёт его приложениям проверки звонков — подставляет пустое значение. '
                'Название видит звонилка, она получает полные данные о звонке, а мы номер. '
                'Поэтому правила по названию срабатывать не могут; блокировать можно по номеру.',
                style: theme.textTheme.bodySmall?.copyWith(
                  color: theme.colorScheme.onSurfaceVariant,
                ),
              ),
            ...
            if (lateOther > 0) ...[
              const SizedBox(height: 12),
              Text(
                'Ещё $lateOther ${plural(lateOther, 'название', 'названия', 'названий')} '
                'появилось позже из системного журнала. Источник там установить нельзя: '
                'телефон записывает туда и имена контактов, и названия, полученные иначе, — '
                'поэтому подписью они не считаются. В момент проверки их всё равно не было.',
                style: theme.textTheme.bodySmall?.copyWith(
                  color: theme.colorScheme.onSurfaceVariant,
                ),
              ),
  ...
  Widget build(BuildContext context) {
    ...
    if (samples.isEmpty) {
      return Padding(
        ...
        child: Text(
          report.checks >= 10
              ? 'Подписей пока не встречалось. При ${report.checks} '
                    '${plural(report.checks, 'проверке', 'проверках', 'проверках')} это уже '
                    'похоже на ответ: либо оператор их не передаёт, либо звонков '
                    'от организаций не было.'
              : 'Подписей приложение пока не наблюдало. При ${report.checks} '
                    '${plural(report.checks, 'проверке', 'проверках', 'проверках')} это ещё '
                    'ничего не значит — вывод имеет смысл делать от десятка звонков.',
          style: theme.textTheme.bodySmall?.copyWith(
            color: theme.colorScheme.onSurfaceVariant,
          ),
        ),
