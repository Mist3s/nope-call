    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // Правила по названию в момент проверки не срабатывают, и об этом надо сказать
          // в редакторе, а не в документации: система не передаёт название приложениям
          // проверки звонков — Telecom подставляет пустое значение
          // (ParcelableCallUtils.toParcelableCallForScreening). Название видит звонилка.
          // Правило всё равно имеет смысл: по нему ищется в журнале, и оно заработает,
          // если платформа когда-нибудь начнёт передавать название.
          if (_target == 'NAME' ||
              _target == 'NAME_ORG' ||
              _target == 'NAME_CATEGORY') ...[
            const SizedBox(height: 12),
            _Note(
              text:
                  'Система не передаёт название звонящего приложениям проверки звонков — '
                  'его видит только звонилка. Поэтому такое правило звонок не заблокирует: '
                  'блокировать можно по номеру. Правило пригодится, чтобы искать такие '
                  'звонки в журнале и увидеть, кого оно зацепило бы.',
