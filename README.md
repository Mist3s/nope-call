...
![Приём](branding/github/social-preview-1280x640.png)

Блокировка входящих звонков по правилам — Android 10 и новее, задел под iOS.
Раздаётся APK напрямую, без Google Play.

Приложение переходит в роль звонилки. Причина в контракте платформы: операторское название
звонящего Android не отдаёт средству проверки звонков вообще, его видит только звонилка
по умолчанию, а правила по названию — единственный способ отбить компанию, у которой номер
меняется каждый день. Пока роль не реализована, работает блокировка по номеру.

Прежний продукт этого репозитория — блокировщик «Отбой» — заморожен тегом `otboy-final`
и больше не развивается.
